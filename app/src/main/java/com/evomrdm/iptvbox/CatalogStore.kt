...
internal class CatalogStore(context: Context) : SQLiteOpenHelper(
    ...
    fun seasons(playlistId: String, seriesTitle: String): List<SeasonGroup> {
        return readableDatabase.rawQuery(
            """
            SELECT
    ...
    fun episodes(playlistId: String, seriesTitle: String, seasonNumber: Int?, limit: Int): List<CatalogItem> {
    ...
    fun categories(playlistId: String, tab: CatalogTab): List<String> {
        val kindPlaceholders = tab.kinds.joinToString(",") { "?" }
        return readableDatabase.rawQuery(
            """
