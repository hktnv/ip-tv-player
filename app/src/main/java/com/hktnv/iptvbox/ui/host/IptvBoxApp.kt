...
internal fun IptvBoxApp(telemetry: AppPerformanceTelemetry) {
    ...
    val catalogSyncController = rememberCatalogSyncController(context, catalogStore)
    val xtreamLazySyncRepository = catalogSyncController.lazySyncRepository
    ...
    StartupCategoryEnrichmentEffect(
        restoredApplied = restoredApplied,
        selectedPlaylist = selectedPlaylist,
        enrichmentQueue = categoryEnrichmentQueue,
        onCatalogChanged = { catalogRefreshToken += 1 },
    )
    CatalogMetadataEffects(
        selectedPlaylist = selectedPlaylist,
        selectedSeriesTitle = selectedSeriesTitle,
        catalogSnapshot = catalogSnapshot,
        catalogRefreshToken = catalogRefreshToken,
        contentOptionsItem = contentOptionsItem,
        lazySyncRepository = xtreamLazySyncRepository,
        onSeriesChanged = { catalogRefreshToken += 1 },
        onContentOptionsMetadata = { contentOptionsMetadata = it },
    )
    ...
    InitialContentFocusEffect(
        restoredApplied = restoredApplied, selectedPlaylist = selectedPlaylist, showPlaylistEntry = showPlaylistEntry,
        screen = screen, initialContentFocusApplied = initialContentFocusApplied,
        onApplied = { initialContentFocusApplied = true }, requestContentFocus = ::requestContentFocus,
    )
    ...
    fun startUpdateDownload(update: AppUpdateInfo) {
        scope.startUpdateDownloadAction(context, update, updateService, updateInstaller, telemetry) { updateState = it }
    }
    ...
    fun deletePlaylist(playlist: LoadedPlaylist) {
        scope.deletePlaylistAction(
            playlist = playlist, catalogStore = catalogStore,
            onDeleted = { deleted ->
                val remaining = playlists.filterNot { it.id == deleted.id }
                val nextState = playlistStateAfterDeletion(remaining, deleted.id, selectedPlaylistId, screen)
                playlists.clear(); playlists.addAll(remaining); favoriteIds.removeAll(deleted.itemIds); recentIds.removeAll(deleted.itemIds)
    ...
    fun selectCatalogTab(tab: CatalogTab) {
        selectedTab = tab; selectedCategory = null; showCatalogCategoryLanding = true
        selectedSeriesTitle = null; selectedSeasonNumber = null; requestContentFocus()
    }
    ...
    fun showCatalogCategories() { selectedSeriesTitle = null; selectedSeasonNumber = null; showCatalogCategoryLanding = true; requestContentFocus() }
    IptvContentHost(
        ...
        onRenamePlaylist = { renamingPlaylist = it }, onDeletePlaylist = { deletingPlaylist = it },
        onAutoUpdateHoursChange = ::updatePlaylistAutoRefresh, onTabSelected = ::selectCatalogTab,
